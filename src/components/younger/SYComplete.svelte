...
<section class="container px-6 py-3 mx-auto">
  ...
  <div class="mx-2 mt-2 md:flex md:items-center md:justify-between grid grid-cols-1 md:grid-cols-2">
    <div class="relative flex items-center">
      ...
      <input type="text" placeholder="Search Model Name" class="block w-full py-1.5 pr-6 text-neutral-800 bg-neutral-200 border border-neutral-300 rounded-lg md:w-80 placeholder-neutral-500 pl-11 dark:bg-neutral-700 dark:text-neutral-300 dark:border-amber-800 focus:border-amber-600 dark:focus:border-amber-600 focus:ring-amber-600 focus:outline-none focus:ring focus:ring-opacity-40" bind:value={search_key} on:keydown={async (event) => {if (event.key === 'Enter') { await getPage(true) }}}>
    </div>
                    ...
                    <td class="px-3 py-3 text-sm text-neutral-500 dark:text-neutral-300 whitespace-nowrap">
                      <div class="flex items-center justify-center gap-x-2">
                        ...
                        <div>
                          ...
                          <a href={instance['model_source'] === 'huggingface' ? model_source_links[instance['model_source']] + instance['model_name'] : model_source_links[instance['model_source']]} class="text-xs font-normal text-sky-500 dark:text-sky-600">See Origin Model</a>
                        </div>
                      </div>
                    </td>
  ...
  {:else}
    <div class="flex items-center mt-6 text-center border rounded-lg h-96 dark:border-neutral-700">
      <div class="flex flex-col w-full max-w-sm px-3 mx-auto">
        ...
        {#if isAuthed}
          {#if search_key}
            <p class="mt-2 text-neutral-500 dark:text-neutral-400">Your search "{search_key}" did not match any Instances. Please try Again.</p>
          ...
          {:else}
            <p class="mt-2 text-neutral-500 dark:text-neutral-400">This version does not contains any Instances. Please try Again.</p>
            <div class="flex items-center mt-3 sm:mx-auto gap-x-3">
          ...
          {/if}
        {:else}
          <p class="mt-2 text-neutral-500 dark:text-neutral-400">Looking For Instances of Complete Series Require GitHub OAuth. Please Sign In.</p>
          <div class="flex items-center mt-3 sm:mx-auto gap-x-3">
  ...
  <div class="mx-2 mt-6 sm:flex sm:items-center sm:justify-between">
    <div class="flex items-center justify-center text-sm font-medium text-neutral-700 dark:text-neutral-300 gap-x-2">
      <div class="relative flex w-full max-w-xs justify-center items-center gap-1 text-slate-700 dark:text-slate-300">
        ...
        <select id="show-count" name="show-count" class="w-[76px] p-0 pl-3 h-6 rounded-md border border-neutral-300 bg-neutral-200 text-sm text-neutral-500 focus:ring-1 focus:ring-purple-300 focus:border-2 focus:border-purple-600 dark:border-purple-800 dark:bg-neutral-700 dark:focus:ring-1 dark:focus:ring-purple-500 dark:focus:border-2 dark:focus:border-purple-800" bind:value={show_count} on:change={() => (console.log(show_count))}>
    ...
    <div class="flex items-center justify-center text-sm font-medium text-neutral-700 dark:text-neutral-300 gap-x-2 mt-3 sm:mt-0">
      ...
      <input id="page" type="text" placeholder="1" class="h-6 -indent-px text-center text-sm block indent-0 aligh-middle w-16 px-2 py-1 text-neutral-800 bg-neutral-200 border border-neutral-300 rounded-md placeholder-neutral-500 dark:bg-neutral-700 dark:text-neutral-300 dark:border-amber-800 focus:border-amber-600 dark:focus:border-amber-600 focus:ring-amber-600 focus:outline-none focus:ring focus:ring-opacity-40" bind:value={page_index} on:input={() => (console.log(page_index))}>
      <span class="text-nowrap">of {total_page}</span>
    </div>
    <div class="flex items-center mt-3 gap-x-2 sm:mt-0">
      <a href="#top" on:click={() => (page_index = Math.max(1, page_index - 1))} class="flex items-center justify-center w-1/2 px-2 py-2 text-sm text-neutral-700 capitalize transition-colors duration-200 bg-neutral-200 border border-neutral-300 shadow-md shadow-neutral-500 dark:shadow-amber-900 rounded-md sm:w-auto gap-x-2 hover:bg-neutral-100 dark:bg-amber-900 dark:text-neutral-200 dark:border-amber-700 dark:hover:bg-amber-800">
      ...
      <a href="#top" on:click={() => (page_index = Math.min(total_page, page_index + 1))} class="flex items-center justify-center w-1/2 px-2 py-2 text-sm text-neutral-700 capitalize transition-colors duration-200 bg-neutral-200 border border-neutral-300 shadow-md shadow-neutral-500 dark:shadow-amber-900 rounded-md sm:w-auto gap-x-2 hover:bg-neutral-100 dark:bg-amber-900 dark:text-neutral-200 dark:border-amber-700 dark:hover:bg-amber-800">
        <span>
          Next
        </span>
