...
<form class="flex w-full items-center justify-center space-y-6 columns-1" on:submit|preventDefault={updateUserInfo}>
  <div class="w-4/5 md:w-3/5 relative -space-y-px rounded-md grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6">
    ...
    <div class="flex w-full justify-center text-nowrap text-ellipsis px-[1px] py-[1px]">
      <button on:click={() => logoutAndRedirect('/signin')} class="m-4 w-1/3 px-1 py-1 border-2 border-purple-300 dark:text-purple-300 hover:bg-purple-500 text-purple-600 hover:text-purple-200 rounded-md text-center transition focus-visible:ring-1 ring-offset-1 ring-purple-200 focus:ring-purple-200">Log Out</button>
    </div>
    {#if showNote}
