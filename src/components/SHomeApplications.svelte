<script>
  ...
  onMount( async () => {
    ...
    homeApplications = await client.request(
      readItems('HomeApplications',
        {
          limit: -1,
...
{#if homeApplications.length}
  {#each homeApplications as homeApplication}
    {#if homeApplication["publish"]}
